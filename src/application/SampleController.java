...
public class SampleController {
	...
	private void initialize() {
		...
		grid.getChildren().stream()
			.forEach(node->{
				...
				button.setOnMouseEntered(val->{
					button.setStyle("-fx-background-color:orange;"
							+ "-fx-text-fill:rgb(51,51,51);");
				});
				button.setOnMouseExited(val->{
					button.setStyle("-fx-background-color:rgb(51,51,51);"
						+ "-fx-text-fill:orange;");
				});
	...
	private void swapButtons(int puzzleColumn, int puzzleRow) {
		...
		//zamieñ teksty przycisków
		empty.setText(button.getText());
		empty.setVisible(true);
		button.setVisible(false);
		button.setText("-1");

		//zamiana indeksów wewnêtrznych
